 ...
 *  @version    2.0.0 (last revision: August 24, 2026)
 *  @copyright  © 2013 - 2026 Stefan Gabos
...
class Zebra_cURL {
     ...
     *      you are downloading a file generated by a script (i.e. https://foo.com/bar.php?w=1200&h=800) or the URL has
     *      no file name in it (i.e. https://foo.com/), the downloaded file's name will be random generated. Refer to the
     *      downloaded file's name in the result's `info` attribute, in the `downloaded_filename` section - see the
     *      example below.
     *
                ...
                while ($info = curl_multi_info_read($this->_multi_handle)) {
                    ...
                    if (isset($request['callback']) && $request['callback'] != '') {
                        ...
                        if (isset($request['download']) && $request['download']) {
                            ...
                            // close the file so that everything is flushed to disk before the callback gets to look at it
                            fclose($request['file_handler']);
                            ...
                            $arguments[0]->info = array_merge($tmp_array, array('downloaded_filename' => $request['file_name']), $arguments[0]->info);
                    ...
                    // if we downloaded a file and there was no callback to close the file for us
                    if (isset($request['download']) && $request['download'] && is_resource($request['file_handler']))
    ...
    private function _queue_requests() {
        ...
        for ($i = 0; $i < min($requests_count, max(1, (int)$this->threads)); $i++) {
            ...
            if (isset($request['download']) && $request['download']) {
                ...
                if (isset($parsed['query']) && $parsed['query'] != '') $file_name = md5($parsed['query']);

                // otherwise use the last segment of the path, decoded (i.e. "my%20file.pdf" is saved as "my file.pdf")
                else $file_name = isset($parsed['path']) ? basename(rawurldecode($parsed['path'])) : '';

                // if there is no usable file name in the URL (i.e. https://foo.com or https://foo.com/), use an md5 of the URL
                if (in_array($file_name, array('', '.', '..'), true)) $file_name = md5($request['url']);

                $request['file_name'] = $request['path'] . $file_name;
